...
from ostilhou.text import correct_sentence
from ostilhou.audio import get_audiofile_length
    ...
    if os.path.exists(filename):
        ...
        with open(filename, 'r') as f:
            ...
            while l:
                ...
                if filtered_out:
                    if speaker_id in blacklisted_speakers or l[3] in blacklisted_sentences:
                        print("blacklisted speaker of sentence found", l)
                        l = f.readline().strip()
                        continue
                
                duration = get_audiofile_length( os.path.join(clip_dir, l[1]) )
                if l[8].startswith("male"):
                    if not speaker_id in speakers:
                        num_m += 1
                        act_num_m += 1
                    dur_m += duration
                    act_dur_m += duration
                elif l[8].startswith("female"):
                    if not speaker_id in speakers:
                        num_f += 1
                        act_num_f += 1
                    dur_f += duration
                ...
                sentences.add(l[3])
                corrected = correct_sentence(l[3])
